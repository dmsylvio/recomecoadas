  ...
  <div class="grid h-screen grid-cols-12 overflow-y-hidden bg-gray-100">
    ...
    >
      <div class="w-full">
        ...
        <div
          class="pt-24 mt-0 text-sm not-italic font-medium leading-relaxed text-left text-gray-400 md:pt-40"
        >
          <p class="mb-3">
            © 2017 - {{ new Date().getFullYear() }}
            <a
              class="text-sm text-primary-400 hover:text-gray-700 underline"
              href="https://switchdesignteam.com"
              >Switch Design Team</a
            >.
          </p>
        </div>
      </div>
    ...
    <div
      class="relative flex-col items-center justify-center hidden w-full h-full pl-10 bg-no-repeat bg-cover md:col-span-6 lg:col-span-8 md:flex content-box overflow-hidden"
    >
      ...
      <div class="md:pl-10 xl:pl-0 relative z-50 w-7/12 xl:w-5/12 xl:w-5/12">
        <h1
          class="hidden mb-3 text-3xl leading-normal text-left text-white xl:text-5xl xl:leading-tight md:none lg:block"
        >
          {{ pageHeading }}
        </h1>
        <p
          class="hidden text-sm not-italic font-normal leading-normal text-left text-gray-100 xl:text-base xl:leading-6 md:none lg:block"
        >
          {{ pageDescription }}
        </p>
      </div>
      ...
      <LoginBottomVector
        class="absolute z-50 w-full bg-no-repeat content-bottom h-[15vw] lg:h-[22vw] right-[32%] bottom-0"
      />
    </div>
  </div>
</template>
...
const pageHeading = computed(() => {
  ...
  return 'Save time, track money and get important insights'
})
...
const pageDescription = computed(() => {
  ...
  return 'Our helps you track expenses, record payments & generate beautiful invoices & estimates.'
})
...
const copyrightText = computed(() => {
  ...
  return 'Copyright @ Switch Design Team.'
})
